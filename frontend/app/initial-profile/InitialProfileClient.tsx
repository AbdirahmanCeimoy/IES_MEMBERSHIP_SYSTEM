...
import { patchStoredUser } from '@/lib/authSession';
...
export const InitialProfileClient = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setLoading(true);
    // TODO: wire to backend profile-create endpoint.
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="border-b border-slate-200 bg-white px-4 py-3">
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          <Link href={routes.home} className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-[#082B55]">{site.shortName}omalia</span>
          </Link>
          <Link href={routes.auth.login} className="text-xs font-semibold text-slate-500 hover:text-[#0047AB]">
            Sign in instead ›
          </Link>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 lg:py-10">
        <div className="mb-5 text-center">
          <h1 className="text-2xl font-bold text-[#082B55]">Initial Profile</h1>
        ...
        >
          ...
          <div className="flex flex-col items-center gap-3 border-b border-slate-100 pb-6 lg:border-b-0 lg:border-r lg:pb-0 lg:pr-6">
            ...
            <label className="w-full">
              <span className="mb-1 block text-[11px] font-semibold text-[#082B55]">Attach Photo *</span>
              <input
                type="file"
                accept=".jpg,.jpeg,.png"
                onChange={(event) => handlePhoto(event.target.files?.[0] ?? null)}
                className="w-full rounded-lg border border-slate-300 px-2 py-1.5 text-[11px] file:mr-2 file:rounded-md file:border-0 file:bg-[#0047AB] file:px-2 file:py-1 file:text-[10px] file:font-semibold file:text-white hover:file:bg-[#082B55]"
              />
              <span className="mt-1 block text-[10px] text-slate-500">PNG and JPEG images only</span>
            </label>
            <label className="w-full">
              <span className="mb-1 block text-[11px] font-semibold text-[#082B55]">Attach ID / Passport *</span>
              <input
                type="file"
                accept=".pdf"
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <label className="flex flex-col gap-1 text-xs font-semibold text-[#082B55] md:col-span-2">
              Discipline *
              <select
                required
            ...
            <div className="md:col-span-2">
              <label className="flex items-center gap-2 text-sm">
                ...
                />
                <span className="text-[#082B55]">Any form of disability</span>
              </label>
              {form.hasDisability && (
                <textarea
                  ...
                  rows={2}
                  className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-normal focus:border-[#0047AB] focus:outline-none focus:ring-1 focus:ring-[#0047AB]"
                />
              )}
            </div>
            ...
            <div className="md:col-span-2 flex justify-end pt-2">
              <button
                type="submit"
                disabled={loading}
                className="inline-flex items-center gap-2 rounded-lg bg-[#66FF00] px-6 py-2.5 text-sm font-semibold text-[#082B55] transition-colors hover:bg-[#5be000] disabled:opacity-50"
              >
        ...
        {showErrors && errors.length > 0 && (
          <>
            ...
            >
              <h2 className="text-base font-semibold text-[#082B55]">Something went wrong</h2>
              ...
              <div className="mt-5 flex justify-end">
                <button
                  type="button"
                  onClick={() => setShowErrors(false)}
                  className="rounded-lg bg-[#0047AB] px-5 py-2 text-sm font-semibold text-white hover:bg-[#082B55]"
                >
                  Try Again
                </button>
              </div>
