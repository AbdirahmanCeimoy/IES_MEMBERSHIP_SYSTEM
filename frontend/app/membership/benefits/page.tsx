          ...
          {benefitsByGrade.map((b) => (
            <Card key={b.grade} padded>
              <div className="mb-3 flex items-center justify-between gap-2">
                <h3 className="text-sm font-semibold text-[#082B55]">{b.grade}</h3>
                {b.postnominal && (
                  <span className="rounded-full border border-blue-100 bg-blue-50 px-2 py-0.5 text-[11px] font-semibold text-[#0047AB]">
                    {b.postnominal}
                  </span>
                )}
              </div>
