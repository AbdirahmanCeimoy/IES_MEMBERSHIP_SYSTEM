      ...
      >
                  ...
                  <Link
                    ...
                    className={cn(
                      ...
                      active
                        ...
                        : 'text-slate-700 hover:bg-slate-100 hover:text-[#0047AB]',
                    )}
                  >
                    {item.icon}
                    {item.label}
        ...
        <div className="border-t border-slate-100 p-3">
          <div className="mb-2 flex items-center gap-2 rounded-lg bg-slate-50 px-2 py-2">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#0047AB] text-xs font-bold text-white">
              {(user.firstName?.[0] ?? user.fullName?.[0] ?? user.username?.[0] ?? 'M').toUpperCase()}
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-semibold text-[#082B55]">
        ...
        <header className="flex h-16 items-center justify-between border-b border-slate-200 bg-white px-4 lg:px-6">
          <div className="flex items-center gap-2">
            ...
            <div className="text-sm text-slate-500">
              Welcome back, <span className="font-semibold text-[#082B55]">{user.fullName ?? user.username}</span>
            </div>
          </div>
          {user.gradeLabel && (
            <span className="hidden rounded-full bg-[#0047AB]/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-[#0047AB] sm:inline-block">
              {user.gradeLabel}
            </span>
          )}
        </header>
